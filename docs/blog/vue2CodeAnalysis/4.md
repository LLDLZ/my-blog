...
- 执行 queueWatcher(this) ， 将 watcher 放入全局的 watcher 队列（ queue ）中 ， 缓存当前 watcher id ， 对进队列的 watcher 进行判重 ， 保证一个 watcher 不会重复入队 ， 入队后将该 watcher 标记为 true 表示当前 watcher 以及入队 。
